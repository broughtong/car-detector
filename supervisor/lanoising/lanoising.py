...
def processBag(path, folder, filename):
    ...
    newScans = []
    for frameIdx in range(len(data["scans"])):
        
        frm = {}
        for scanner, vals in data["scans"][frameIdx].items():
            if len(vals):
                frm[scanner] = callback(data["scans"][frameIdx][scanner])
            else:
                frm[scanner] = []
...
if __name__ == '__main__':
    ...
    gpr1_disappear_path = './models/gpr1_disappear.m'
    gpr2_disappear_path = './models/gpr2_disappear.m'
    gpr1_disappear = joblib.load(gpr1_disappear_path)
    gpr2_disappear = joblib.load(gpr2_disappear_path)
    mdn1_disappear_path = './models/mdn1_disappear.h5'
    mdn2_disappear_path = './models/mdn2_disappear.h5'
    ...
    gpr1_range_path = './models/gpr1_range.m'
    gpr2_range_path = './models/gpr2_range.m'
    gpr1_range = joblib.load(gpr1_range_path)
    gpr2_range = joblib.load(gpr2_range_path)
    mdn1_range_path = './models/mdn1_range.h5'
    mdn2_range_path = './models/mdn2_range.h5'
    ...
    gpr1_intensity_path = './models/gpr1_intensity.m'
    gpr2_intensity_path = './models/gpr2_intensity.m'
    gpr1_intensity = joblib.load(gpr1_intensity_path)
    gpr2_intensity = joblib.load(gpr2_intensity_path)
    mdn1_intensity_path = './models/mdn1_intensity.h5'
    mdn2_intensity_path = './models/mdn2_intensity.h5'
